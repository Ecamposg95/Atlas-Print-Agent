...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Falló: python -m pip install --upgrade pyinstaller" }
    python -m pip install --quiet -r requirements-labels.txt
    if ($LASTEXITCODE -ne 0) { throw "Falló: python -m pip install -r requirements-labels.txt" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Falló: python -m PyInstaller" }
